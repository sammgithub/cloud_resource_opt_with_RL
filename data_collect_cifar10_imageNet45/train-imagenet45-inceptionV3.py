...
def get_inception_v3(num_classes=45):
    # Instantiate Inception v3 with aux_logits disabled.
    model = models.inception_v3(pretrained=False, aux_logits=False, num_classes=num_classes)
    return model

def train_imagenet45_inception_v3():
    ...
    config = {
        ...
        'dataset': 'imagenet45',
        'batch_size': 128 * max(1, n_gpus),  # Scale batch size with GPU count
        'epochs': 100,
        'learning_rate': 0.001,
    ...
    # Define transform: Resize images to 299x299, convert to tensor, and normalize using ImageNet stats.
    ...
    model = get_inception_v3(num_classes=45).to(device)
    ...
    for epoch in range(config['epochs']):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            # Inception v3 forward pass with aux_logits disabled returns a single output.
            output = model(data)
        ...
        metrics_df.to_csv('log-imagenet45-inceptionV3.csv', index=False)
