...
def get_mobilenet(num_classes=45):
    # Use MobileNetV3-Large from torchvision.
    # This variant expects 224x224 images.
    model = models.mobilenet_v3_large(pretrained=False, num_classes=num_classes)
    return model

def train_imagenet45_mobilenet():
    ...
    # Define transform: Resize images to 224x224, then normalize.
    ...
    # Load dataset from /home/ec2-user/imagenet45 using ImageFolder.
    ...
    model = get_mobilenet(num_classes=45).to(device)
    ...
    metrics_df = pd.DataFrame(columns=[
        'timestamp', 'epoch', 'model_name', 'model_params', 'model_layers', 'dataset',
        'data_count', 'data_size_mb', 'data_dim', 'instance_type', 'device_type',
        'cpu_count', 'gpu_count', 'train_loss', 'train_accuracy', 'test_loss',
        'test_accuracy', 'epoch_time', 'time_so_far', 'cpu_percent', 'memory_used_gb',
        'gpu_memory_used_gb', 'gpu_utilization', 'throughput', 'cost_so_far'
    ])
    ...
    for epoch in range(config['epochs']):
        ...
        metrics_df.to_csv('log-imagenet45-mobileNetV3lrg.csv', index=False)
