...
def get_efficientnet_b5(num_classes=45):
    # Instantiate EfficientNet-B5 from torchvision.
    # Recommended input size for EfficientNet-B5 is 456x456.
    model = models.efficientnet_b5(pretrained=False, num_classes=num_classes)
    return model

def train_imagenet45_efficientnet_b5():
    ...
    config = {
        ...
        'dataset': 'imagenet45',
        'batch_size': 16 * max(1, n_gpus),  # Scale batch size with number of GPUs
        'epochs': 100,
        'learning_rate': 0.001,
    ...
    # Define transforms: Resize images to 456x456, then convert to tensor and normalize.
    ...
    model = get_efficientnet_b5(num_classes=45).to(device)
    ...
    for epoch in range(config['epochs']):
        ...
        metrics_df.to_csv('log-imagenet45-efficientnetB5.csv', index=False)
...
if __name__ == "__main__":
    train_imagenet45_efficientnet_b5()
