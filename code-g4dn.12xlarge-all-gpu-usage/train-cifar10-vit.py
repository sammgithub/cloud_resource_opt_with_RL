...
def get_system_info():
    ...
    return {
        ...
        'gpu_utilizations': gpu_utilizations  # Note: plural key now!
    }

def get_vit(num_classes=10):
    # Use the Vision Transformer model from torchvision.
    # vit_b_16 expects 224x224 images.
...
def train_vit():
    ...
    config = {
        'model_name': 'vit_b_16',
        'dataset': 'cifar10',
        'batch_size': 128 * max(1, n_gpus),  # scale with GPUs
        'epochs': 100,
        'learning_rate': 0.001,
    ...
    # Define transform to resize CIFAR10 (32x32) to 224x224 as expected by ViT.
    ...
    model = get_vit(num_classes=10).to(device)
    if n_gpus > 1:
    ...
    for epoch in range(config['epochs']):
        ...
        metrics_df.to_csv('log-cifar10-vit.csv', index=False)
