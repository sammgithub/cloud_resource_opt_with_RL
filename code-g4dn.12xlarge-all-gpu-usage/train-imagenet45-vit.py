...
def get_system_info():
    ...
    return {
        ...
        'gpu_utilizations': gpu_utilizations  # Use plural key here
    }

def get_vit(num_classes=45):
    # Load Vision Transformer (vit_b_16) from torchvision
    model = models.vit_b_16(pretrained=False, num_classes=num_classes)
    return model

def train_imagenet45_vit():
    ...
    # Define transform: Resize images to 224x224, convert to tensor, and normalize.
    ...
    model = get_vit(num_classes=45).to(device)
    ...
    for epoch in range(config['epochs']):
        ...
        metrics_df.to_csv('log-imagenet45-vit.csv', index=False)
