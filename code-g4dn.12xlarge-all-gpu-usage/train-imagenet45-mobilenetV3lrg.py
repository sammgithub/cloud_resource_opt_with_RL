...
def get_system_info():
    gpu_memory_used = 0
    gpu_utilizations = get_gpu_utilizations()  # List of utilization values for all GPUs
...
def get_mobilenet_v3_large(num_classes=10):
    # Load MobileNetV3-Large from torchvision with specified num_classes
...
def train_mobilenet_v3():
    ...
    # Training configuration for MobileNetV3-Large on CIFAR10
    config = {
        'model_name': 'mobilenet_v3_large',
        'dataset': 'cifar10',
        'batch_size': 128 * max(1, n_gpus),  # Scale batch size with GPUs
        'epochs': 100,
        'learning_rate': 0.001,
    ...
    # Define transform: Resize CIFAR10 images from 32x32 to 224x224, then normalize.
    ...
    model = get_mobilenet_v3_large(num_classes=10).to(device)
    if n_gpus > 1:
    ...
    metrics_df = pd.DataFrame(columns=[
        ...
        'dataset', 'data_count', 'data_size_mb', 'instance_type', 'device_type',
        'cpu_count', 'gpu_count', 'train_loss', 'train_accuracy', 'test_loss',
        'test_accuracy', 'epoch_time', 'time_so_far', 'cpu_percent', 'memory_used_gb',
        'gpu_memory_used_gb', 'throughput', 'cost_so_far'
    ])
    ...
    for epoch in range(config['epochs']):
        ...
        metrics_df.to_csv('log-cifar10-mobileNetV3.csv', index=False)
