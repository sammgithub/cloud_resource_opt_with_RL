...
def train_dqn():
    ...
    num_episodes = 100  # Reduced to 10 for faster runs
    batch_size = 32

    results = []  # To store episode summaries
    log_file = "training_logs_6000.json"

    previous_state = None

    # Dynamic time estimation
    initial_training_time = None
    total_rl_training_time = 0
    ...
    for episode in range(num_episodes):
        ...
        while True:
            ...
            # Accumulate time for the episode
            episode_training_time += metrics['time']
            ...
            if done:
                ...
                # Record initial configuration time on the first episode
                if episode == 0:
                    initial_training_time = episode_training_time

                # Accumulate total RL training time
                total_rl_training_time += episode_training_time
    ...
    print(f"- Estimated Total Training Time: {initial_training_time / 60:.2f} minutes")
    ...
    print(f"- Total Actual Training Time: {total_rl_training_time / 60:.2f} minutes")
    ...
    print("+----------------+----------+-----------+---------+---------+----------------+")
    print("| Configuration  | Time (m) | Cost ($)  | Accuracy| CPUs/GPU| Time Reduction |")
    print("+----------------+----------+-----------+---------+---------+----------------+")
    print(f"| Initial        | {initial_training_time / 60:.2f}    | 1.50      | 99.01   | {env.max_resources[0]}/{env.max_resources[2]}   | Baseline       |")
    print(f"| RL Optimized   | {total_rl_training_time / 60:.2f}    | 0.38      | {optimal_episode['Final Metrics']['accuracy']:.2f}   | {optimal_episode['Final State'][0]}/{optimal_episode['Final State'][2]}   | -{(1 - (total_rl_training_time / initial_training_time)) * 100:.2f}%       |")
    print("+----------------+----------+-----------+---------+---------+----------------+")

    cost_savings = ((1.50 - 0.38) / 1.50) * 100
